...
const Page = () => {
  ...
  // Selected files state (UI only, no upload)
  const [selectedFiles, setSelectedFiles] = useState<File[]>([]);
  const MAX_ATTACHMENTS = 10; // still here but effectively only 1 is used now
  ...
  const handleSendMessage = async () => {
    ...
    const optimisticPost: PostWithReactions = {
      ...
      pinnedAt: null,
      media: null,
      createdAt: new Date(), // PostWithReactions expects Date | null
      updatedAt: null,
      reactions: [],
    };
  ...
  // Only ONE media/file allowed at a time
  const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    const files = Array.from(e.target.files || []);
    if (!files.length) return;

    // Always keep only the first selected file
          ...
          <div className="flex flex-col max-w-3xl w-full gap-y-2">
              ...
              <div className="flex flex-wrap gap-x-2">
                {selectedFiles.map((file, index) => {
                  ...
                  return (
                    <div
                      key={`${file.name}-${index}`}
                      className="relative cursor-pointer flex items-center gap-2 rounded-xl border border-primary/5 bg-primary/1 px-1 py-1 mt-2 text-xs text-primary"
                    >
                      {/* Thumbnail / icon */}
                      <div className="w-12 h-12 rounded-lg overflow-hidden border border-primary/5 bg-primary/3 flex items-center justify-center text-sm">
                        {isImage ? (
                          <img
                            src={objectUrl}
                            alt={file.name}
            ...
            <div className="flex gap-x-2 w-full items-end">
              ...
              <label className="relative">
                <input
                  type="file"
                  // single selection on all devices
                  accept="image/*,video/*,audio/*,application/*"
                  className="hidden"
                  onChange={handleFileChange}
                />
                <GalleryVerticalEndIcon className="stroke-[1px] flex justify-center items-center opacity-80 w-10 h-10 mt-3 lg:mt-5 border border-secondary/3 p-2 rounded-full cursor-pointer" />
              </label>
